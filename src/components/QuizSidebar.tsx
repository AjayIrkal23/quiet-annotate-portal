...
const QuizSidebar: React.FC<QuizSidebarProps> = ({
  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-4 border border-gray-700/50 min-w-[260px] w-full mb-4 xl:mb-0">
        ...
        {boundingBoxes.length === 0 ? (
          ...
          boundingBoxes.map(box => {
            ...
            return (
              <div
                key={box.id}
                className={`rounded-lg p-3 border ${bg} transition-colors duration-300`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    {/* Colored dot (always grey, since we hide the issue name) */}
                  ...
                  {/* No delete/trash or label for issues */}
                </div>
              </div>
            );
          })
